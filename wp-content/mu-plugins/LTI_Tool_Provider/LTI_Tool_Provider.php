...
// Check consumer key
    ...
    if ($this->isOK) {
	  ...
	  //error_log("og oath failed: ".print_r($e,true));
...
#
      ...
      //}
      //error_log("og post:".print_r($_POST,true));
      if (isset($_POST['lis_person_sourcedid'])) {
        $user_id = trim($_POST['lis_person_sourcedid']);
      }
      //error_log("og userid: ".$user_id);
  ...
  public function doMembershipsService($withGroups = FALSE) {
    ...
    if ($ok) {
      ...
      for ($i = 0; $i < count($members); $i++) {
        ...
        //error_log("og LTI member: ".$members[$i]['lis_person_sourcedid']);
	//$user = new LTI_User($this, $members[$i]['user_id']);
	$user = new LTI_User($this, $members[$i]['lis_person_sourcedid']);
#
### Set the user name
#
 ...
 */
  public function getId($id_scope = NULL) {
    ...
    //error_log("og LTI concat id:". $id. " id: ".$this->id);
    return $id;
