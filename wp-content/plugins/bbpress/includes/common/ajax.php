...
function bbp_do_ajax() {
	...
	//error_log("og do ajax");
	// Bail if not an ajax request
	if ( ! bbp_is_ajax() )
		return;
	//error_log("og do ajax true ".'bbp_ajax_' . $_REQUEST['action']);
...
function bbp_ajax_response( $success = false, $content = '', $status = -1, $extras = array() ) {
	error_log("og ajax success: ".$successi." status: ".$status." 404: ".print_r(http_response_code(),true));
	// Set status to 200 if setting response as successful
	if ( ( true === $success ) && ( -1 === $status ) )
		$status = 200;

	// og: For some reason making ajax calls from the group forum pages results in a 404 http code even though the returned content is fine and it says 200.
	// This code corrects this problem
	if(($status==200)&&(http_response_code()==404)){
		// source: http://stackoverflow.com/questions/948116/ajax-404-with-results-using-buddypress
		status_header( 200 );
		error_log("og ajax setting to 200");
	} // end og
