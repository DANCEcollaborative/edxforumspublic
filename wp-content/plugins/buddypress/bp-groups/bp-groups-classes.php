...
class BP_Group_Extension {
	...
	protected function setup_create_hooks() {
		...
		// I tried to set enable_nav_item to false, but it did nothing
		//$screen["screen_callback"]->enable_nav_item = false;
		//$screen["screen_callback"]->params->enable_nav_item = false;

		// Remove forum from the tabs (it will be created. The user no longer gets the option to choose whether they want to have a group forum or not).
		...
		// Directly make the creation steps action call the function create_screen_save_notab.
		// This function does not expect any POST arguments (as opposed to create_screen_save) because it assumes that the group creation forum tab is hidden.
		add_action( 'groups_custom_create_steps', array( $this, 'maybe_create_screen_save_notab' ) );
	...
	/**
         * Call the create_screen_save_notab() method, if we're on the right page (group settings in this case). Unlike maybe_create_screen_save, this function assumes that
	 * the group creation page has no forum tab and that new groups always need to have a forum.
         *
         * @since BuddyPress (1.8.0)
         */
        public function maybe_create_screen_save_notab() {
                if ( ! bp_is_group_creation_step( 'group-settings' ) ) {
			error_log("og not group settings");
                        return;
                }
		error_log("og is group settings");

                // The're no need to check the nonce since we removed the forum tab 
                call_user_func( array($this, 'create_screen_save_notab'), $this->group_id );
        }
