...
class BP_Legacy extends BP_Theme_Compat {
	...
	protected function setup_actions() {
		...
		$actions = array(
			...
			// og added
			'my_get_friendship_status'  => 'bp_legacy_theme_ajax_my_get_friendship_status',
			...
			// og added
			'my_is_user_in_group' => 'bp_legacy_theme_ajax_my_is_user_in_group',
			'my_is_user_group_admin' => 'bp_legacy_theme_ajax_my_is_user_group_admin',
			'get_joinleave_group_status' => 'bp_legacy_theme_ajax_get_joinleave_group_status',
...
function bp_legacy_theme_ajax_my_is_user_in_group(){
	$mygroupname=$_POST['groupname'];
	$mygroupid=(int)groups_get_id($mygroupname);
	error_log("og in get group ".$mygroupname." ".groups_get_id($mygroupname));
	if ( ! $group = groups_get_group( array( 'group_id' => $mygroupid ) ) )
                return;
	if(groups_is_user_member( bp_loggedin_user_id(),$group->id)){
		echo "1";
		error_log("og in get group user is in group");
	}
	else{
		echo "0";
		error_log("og in get group no 0");
	}
	exit;
}
...
function bp_legacy_theme_ajax_get_joinleave_group_status() {
	...
	error_log("og group status:".print_r($my_group_ids,true));
		...
		$group_id = (int) $my_group_ids[$myidx];//$_POST['gid'];
		error_log("og group id:".$group_id." ");
		$bp_loggedin_user_id = bp_loggedin_user_id();

        	if ( groups_is_user_banned( $bp_loggedin_user_id, $group_id ) ){
                	$myretval[$my_group_ids[$myidx]] = "";
		} // populate_extras obtains extra info that enables bp_get_group_join_button to check if the current user is the group admin
        	else if ( $group = groups_get_group( array( 'group_id' => $group_id, 'populate_extras' => true ) ) ){
        	        //return;
...
function bp_legacy_theme_ajax_get_user_joinleave_group_status($group) {
	error_log("og group status is member:".$group->is_member);
	return bp_get_group_join_button($group);
}

// This is an older version of bp_legacy_theme_ajax_get_user_joinleave_group_status that does not require groups_get_group to get the extras. 
// However, it is less correct since it allows sole group admins to leave their groups.
	...
	// Not a member
        if ( ! groups_is_user_member( bp_loggedin_user_id(), $group->id ) ) {
			...
			// The user requested membership and the request is pending
				...
				return '<div class="generic-button group-button pending private" id="groupbutton-'.esc_attr( $group->id ).'"><a id="group-' . esc_attr( $group->id ) . '" class="group-button membership-requested" rel="membership-requested" title="' . __( 'Membership Requested', 'buddypress' ) .
 '" href="' . bp_get_group_permalink( $group ) . '">' . __( 'Membership Requested', 'buddypress' ) . '</a>'.'</div>';
