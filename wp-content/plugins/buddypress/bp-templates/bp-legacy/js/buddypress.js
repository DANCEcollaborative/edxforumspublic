...
jq(document).ready( function() {
	...
	// Make sure to use ajax to get the activity content in order to override the things that come from the cache
	//if(jq('#activity-filter-select').length>0){
	//	if(jq("p.activity-greeting").length==1){
	//		jq("p.activity-greeting").text(jq("p.activity-greeting").text().replace(/(.*\w,).*(\?)/,"$1 "+jq.cookie("uname")+"$2"));
	//	}
	//	jq('#activity-filter-select').find("option[value=-1]").attr('selected', 'selected');
